...
import ai.medusa.anticheat.data.processor.VelocityProcessor;
...
/**
 * Created on 12/19/2020 Package ai.medusa.anticheat.check.impl.movement.speed by infinitesm
 */

@CheckInfo(name = "Speed (B)", description = "Checks for movement speed.", experimental = true)
public final class SpeedB extends Check {
    ...
    public void handle(final Packet packet) {
        if (packet.isPosition() && !isExempt(ExemptType.TELEPORT, ExemptType.FLYING)) {
            ...
            final boolean exempt = isExempt(ExemptType.STAIRS, ExemptType.LIQUID, ExemptType.PISTON, ExemptType.TELEPORT);

            if (shiftedSpeed > 100) {
                if ((buffer += shiftedSpeed > 300 ? 60 : 20) > 100 || shiftedSpeed > 1000) {
                    if (!exempt) {
                        fail(String.format(
                                "speed=%o%%, buffer=%.2f",
